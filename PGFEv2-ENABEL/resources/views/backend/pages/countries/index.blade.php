...
@section('admin-content')
    <x-admin.shadcn-shell
        title="Pays"
        subtitle="Pays disponibles dans le système."
        icon="lucide:globe"
        breadcrumbCurrent="Liste"
    >
        <x-slot:actions>
            <a href="{{ route('admin.countries.create') }}" class="admin-btn-primary">
                <iconify-icon icon="lucide:plus-circle" width="16"></iconify-icon>
                Nouveau pays
            </a>
        </x-slot:actions>

        <x-admin.crud-table :colspan="3">
            <x-slot:head>
            ...
            <x-slot:body>
                    ...
                    <tr class="transition-colors hover:bg-zinc-50/50">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('admin.countries.edit', $c) }}"
                                ...
                                <form action="{{ route('admin.countries.destroy', $c) }}" method="POST" onsubmit="return confirm('Supprimer ce pays ?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                ...
                @empty
                    <tr>
                        <td colspan="3" class="px-6 py-16 text-center text-sm text-zinc-500">Aucun pays enregistré.</td>
                    </tr>
                @endforelse
            </x-slot:body>
            @if($countries->hasPages())
                <x-slot:pagination>{{ $countries->links() }}</x-slot:pagination>
            @endif
        </x-admin.crud-table>
    </x-admin.shadcn-shell>
@endsection
