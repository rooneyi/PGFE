...
@section('admin-content')
    <div class="space-y-6">
        <nav class="flex items-center gap-2 text-xs font-semibold text-gray-500 uppercase tracking-widest">
            <!-- BREADCRUMBS: [['title' => 'Inventaires Infrastructure', 'url' => route('admin.infra-infrastructure-inventaires.index')], ['title' => 'Nouveau']] -->
        </nav>
    </div>
    ...
    <div class="max-w-3xl mx-auto space-y-10">
        ...
        <div class="flex items-center justify-between bg-white dark:bg-gray-900 p-8 rounded-[2.5rem] shadow-xl border border-gray-100 dark:border-gray-800">
            <div class="flex items-center gap-6">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-500 font-medium mt-1">Évaluation et état des lieux d'une infrastructure.</p>
                </div>
            </div>
        ...
        <div class="bg-white dark:bg-gray-900 rounded-[2.5rem] shadow-xl border border-gray-100 dark:border-gray-800 overflow-hidden">
            <form action="{{ route('admin.infra-infrastructure-inventaires.store') }}" method="POST" class="p-10 space-y-8">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="space-y-2 md:col-span-2">
                        ...
                        <input type="text" name="title" required placeholder="ex: Audit Trimestriel - Bâtiment Administratif" 
                            class="w-full h-14 px-5 rounded-2xl border-gray-100 dark:border-gray-800 dark:bg-gray-950 focus:ring-indigo-500 font-bold transition-all" value="{{ old('title') }}" />
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="infra_infrastructure_id" required class="w-full h-14 px-5 rounded-2xl border-gray-100 dark:border-gray-800 dark:bg-gray-950 focus:ring-indigo-500 font-bold appearance-none transition-all">
                            ...
                            @foreach($infrastructures as $infra)
                                <option value="{{ $infra->id }}" @selected(old('infra_infrastructure_id') == $infra->id)>{{ $infra->name }} ({{ $infra->code }})</option>
                            @endforeach
                        </select>
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="date" name="inventory_date" required 
                            class="w-full h-14 px-5 rounded-2xl border-gray-100 dark:border-gray-800 dark:bg-gray-950 focus:ring-indigo-500 font-bold transition-all" value="{{ old('inventory_date', date('Y-m-d')) }}" />
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="status" required class="w-full h-14 px-5 rounded-2xl border-gray-100 dark:border-gray-800 dark:bg-gray-950 focus:ring-indigo-500 font-bold appearance-none transition-all">
                            <option value="excellent" @selected(old('status') == 'excellent')>Excellent</option>
                            <option value="bon" @selected(old('status', 'bon') == 'bon')>Bon</option>
                            <option value="moyen" @selected(old('status') == 'moyen')>Moyen (Usure normale)</option>
                            <option value="mauvais" @selected(old('status') == 'mauvais')>Mauvais (Réparation nécessaire)</option>
                            <option value="critique" @selected(old('status') == 'critique')>Critique (Danger/Inutilisable)</option>
                        </select>
                    ...
                    <div class="space-y-2 md:col-span-2">
                        ...
                        <textarea name="description" rows="4" 
                            ...
                            placeholder="Détails sur l'état des lieux, les besoins de maintenance vus lors de la visite...">{{ old('description') }}</textarea>
                    </div>
                </div>
