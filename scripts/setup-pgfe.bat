...
echo   Projet : %ROOT%
echo.
echo Ce script prepare le backend et le frontend.
echo Prerequisites : Laragon, Node.js, Git deja installes.
echo.
...
where php >nul 2>&1
if errorlevel 1 (
  echo [ERREUR] php introuvable. Installez PHP 8.4+ ^(Laragon ou php.net^) puis relancez.
  pause
  exit /b 1
)
...
if errorlevel 1 (
  ...
  php -v
  echo          Laragon -^> Menu PHP -^> choisissez PHP 8.4, ou installez PHP 8.4.
  pause
  exit /b 1
)
...
where npm >nul 2>&1
if errorlevel 1 (
  echo [ERREUR] npm introuvable. Installez Node.js LTS puis relancez.
  pause
  exit /b 1
)
...
powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0create-shortcut.ps1" -AlsoDesktop
if errorlevel 1 (
...
echo     Double-cliquez sur PGFE.lnk
echo     ^(a la racine du projet ou sur le Bureau^)
echo.
