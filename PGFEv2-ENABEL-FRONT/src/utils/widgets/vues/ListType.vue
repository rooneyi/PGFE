<template>
  <div class="flex flex-col space-y-1.5 flex-1">
    ...
    <Select id="type_id" name="type_id" :model-value="modelValue" @update:modelValue="updateValue">
      <SelectTrigger id="type_id" class="h-10 w-full">
      ...
      <SelectContent>
        <SelectGroup>
          ...
          <SelectItem else v-for="type in data" :key="type.id" :value="String(type.id)">
            {{ type.title }}
          </SelectItem>
        </SelectGroup>
      </SelectContent>
...
const { data, loading, error, fetchData } = useGetApi<Type[]>(API_ROUTES.GET_TYPES)
fetchData()
