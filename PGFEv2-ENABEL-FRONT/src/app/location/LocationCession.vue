...
const { data: apiData, loading, fetchData } = useGetApi<Session[]>(API_ROUTES.GET_RENTAL_SESSIONS)
...
const filteredItems = computed(() => {
  ...
  return allItems.value.filter((item) => {
    return (
      q.length === 0 ||
      (item.session_code && item.session_code.toLowerCase().includes(q)) ||
...
const handleDelete = async (id: number) => {
  await deleteData(API_ROUTES.DELETE_RENTAL_SESSION(id))
  await fetchData()
}
  ...
  <DashLayout module-name="location" active-route="/location/operations" :breadcrumb="breadcrumb">
    ...
    <BoxPanelWrapper>
      <div class="flex sm:items-center gap-3 flex-col sm:flex-row sm:justify-between">
        ...
        <div class="flex flex-wrap items-center sm:justify-end gap-2.5 flex-1">
          <Button size="md" class="rounded-md max-sm:flex-1 sm:w-max" as-child>
            <RouterLink to="/location/operations/cessions/nouveau">
              <span class="flex iconify hugeicons--plus-sign"></span>
              <span class="hidden sm:flex">Nouvelle session</span>
            </RouterLink>
          </Button>
        ...
        <Table class="rounded-md bg-white">
          ...
          <TableBody>
            <TableRow v-for="item in data" :key="item.id">
              ...
              <TableCell>{{ item.session_code || '-' }}</TableCell>
              ...
              <TableCell>
                <div class="group flex items-center justify-between">
                  ...
                  <div class="hidden group-hover:flex items-center gap-2 ml-2">
                    ...
                    >
                      <RouterLink to="/location/operations/cessions">
                    ...
                    <AlertMessage
                      ...
                      :message="`Vous êtes sur le point de supprimer la session ${item.session_code || 'sans code'}. Etes-vous sur de continuer?`"
                    >
                      <template #trigger>
                        <Button
                          variant="ghost"
      ...
      >
        ...
        <span>Aucune session trouvée pour le moment.</span>
      </div>
