...
const { data: apiData, loading, fetchData } = useGetApi<Payment[]>(API_ROUTES.GET_RENTAL_PAYMENTS)
...
const filteredItems = computed(() => {
  ...
  return allItems.value.filter((item) => {
    return (
      q.length === 0 ||
      item.rental_contract_id.toLowerCase().includes(q) ||
...
const handleDelete = async (id: number) => {
  await deleteData(API_ROUTES.DELETE_RENTAL_PAYMENT(id))
  await fetchData()
}
...
const breadcrumb: BreadcrumbProps = {
  items: [
    ...
    { label: 'Versements', isActive: true },
  ],
}
</script>
  ...
  <DashLayout module-name="location" active-route="/location/operations" :breadcrumb="breadcrumb">
    ...
    <BoxPanelWrapper>
      <div class="flex sm:items-center gap-3 flex-col sm:flex-row sm:justify-between">
        ...
        <div class="flex flex-wrap items-center sm:justify-end gap-2.5 flex-1">
          <Button size="md" class="rounded-md max-sm:flex-1 sm:w-max" as-child>
            <RouterLink to="/location/operations/versements/nouveau">
              <span class="flex iconify hugeicons--plus-sign"></span>
              <span class="hidden sm:flex">Nouveau versement</span>
            </RouterLink>
          </Button>
        ...
        <Table class="rounded-md bg-white">
          ...
          <TableBody>
            <TableRow v-for="item in data" :key="item.id">
              ...
              <TableCell>{{ item.rental_contract_id }}</TableCell>
              <TableCell>{{ Number(item.amount).toLocaleString('fr-FR') }} $</TableCell>
              ...
              <TableCell>
                <div class="group flex items-center justify-between">
                  ...
                  <div class="hidden group-hover:flex items-center gap-2 ml-2">
                    ...
                    >
                      <RouterLink to="/location/operations/versements">
                    ...
                    <AlertMessage
                      ...
                      :message="`Vous êtes sur le point de supprimer ce versement de ${Number(item.amount).toLocaleString('fr-FR')} $. Etes-vous sur de continuer?`"
                    >
                      <template #trigger>
                        <Button
                          variant="ghost"
      ...
      >
        ...
        <span>Aucun versement trouvé pour le moment.</span>
      </div>
