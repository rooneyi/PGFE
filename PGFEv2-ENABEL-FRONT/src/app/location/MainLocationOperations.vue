...
import { tagLocationNavSaisie } from '@/components/templates/location/location-tags-links'
...
type Contract = {
  id: number
  client_id: number
  client?: { id: number; name?: string; nom?: string }
  start_date: string
  end_date: string
  status: string
}
...
// API calls
const { data: rawData, loading, fetchData } = useGetApi(API_ROUTES.GET_RENTAL_CONTRACTS)
...
const filteredItems = computed(() => {
  ...
  return contracts.value.filter((item: Contract) => {
    if (q.length === 0) return true
    const clientName = item.client?.name || item.client?.nom || ''
...
const handleDelete = async (id: number) => {
  await deleteItem(API_ROUTES.DELETE_RENTAL_CONTRACT(id))
  if (deleteSuccess.value) {
    showCustomToast({ message: 'Contrat supprimé avec succès', type: 'success' })
    fetchData()
  }
}
...
const breadcrumb: BreadcrumbProps = {
  items: [
    ...
    { label: 'Contrats', isActive: true },
  ],
}
</script>

<template>
  <DashLayout module-name="location" active-route="/location/operations" :breadcrumb="breadcrumb">
    <div class="pb-6 mx-auto w-full max-w-7xl">
      <DashPageHeader
        title="Location"
        :tags="tagLocationNavSaisie"
        active-tag-name="contrat"
      />
    <BoxPanelWrapper>
      <div class="flex sm:items-center gap-3 flex-col sm:flex-row sm:justify-between">
        <div class="flex flex-1 items-center gap-2">
          <div class="relative w-full max-w-xs">
            <Input
              ...
              id="search"
              name="search"
              placeholder="Rechercher un contrat ou locataire..."
        ...
        <div class="flex flex-wrap items-center sm:justify-end gap-2.5 flex-1">
          <Button size="md" class="rounded-md max-sm:flex-1 sm:w-max" as-child>
            <RouterLink to="/location/operations/contrats/nouveau">
              <span class="flex iconify hugeicons--plus-sign"></span>
              <span class="hidden sm:flex">Nouveau contrat</span>
            </RouterLink>
          </Button>
        ...
        <Table class="rounded-md bg-white">
          ...
          <TableBody>
            <TableRow v-for="item in data" :key="item.id">
              ...
              <TableCell class="font-medium">{{ item.client?.name || item.client?.nom || `Client #${item.client_id}` }}</TableCell>
                ...
                <div class="group flex items-center justify-between">
                  ...
                  <div class="hidden group-hover:flex items-center gap-2 ml-2">
                    ...
                    <AlertMessage
                      ...
                      :message="`Vous êtes sur le point de supprimer ce contrat. Êtes-vous sûr de continuer?`"
                    >
                      <template #trigger>
                        <Button
                          variant="ghost"
      ...
      >
        ...
        <span>Aucun contrat trouvé pour le moment.</span>
      </div>
