...
const activeTagName = 'fournisseurs'

const query = ref('')
const { data: rawData, loading, fetchData, meta } = useGetApi(API_ROUTES.GET_STOCK_PROVIDERS)
...
const filteredProviders = computed(() => {
  if (!query.value) return providers.value
...
eventBus.on('stockProviderUpdated', () => {
  fetchData({ page: page.value, limit: perPageCount.value })
})
...
const handleDelete = async (id: number) => {
  deletingId.value = id
  const url = API_ROUTES.DELETE_STOCK_PROVIDER(id)
...
const handleAddFournisseur = () => {
  router.push('/stock/prealables/fournisseurs/nouveau')
}
</script>
      ...
      <BoxPanelWrapper class="flex-1 flex flex-col min-h-0">
        <div class="flex sm:items-center gap-3 flex-col sm:flex-row sm:justify-between mb-4">
          ...
          <div class="flex flex-wrap items-center sm:justify-end gap-2.5 flex-1">
            <Button
              size="md"
              class="rounded-md max-sm:flex-1 sm:w-max"
              @click="handleAddFournisseur"
            >
              <span class="flex iconify hugeicons--plus-sign"></span>
              <span class="hidden sm:flex">Ajouter Fournisseur</span>
            </Button>
        ...
        <div
          v-else-if="filteredProviders && filteredProviders.length"
          ...
          <Table class="rounded-md bg-white">
            <TableHeader>
              <TableRow>
                ...
                <TableHead>Contact (Tél/Email)</TableHead>
                <TableHead>Adresse</TableHead>
                <TableHead class="text-right">Actions</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              <TableRow v-for="(item, index) in filteredProviders" :key="item.id">
