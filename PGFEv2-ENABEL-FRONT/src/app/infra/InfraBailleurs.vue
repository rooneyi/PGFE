...
// API calls
const { data: apiData, loading, fetchData } = useGetApi<InfraBailleur[]>(
  API_ROUTES.GET_INFRA_BAILLEURS
)
...
const handleDelete = async (id: number) => {
  await deleteItem(API_ROUTES.DELETE_INFRA_BAILLEUR(id))
...
const handleSubmit = async () => {
  if (!formData.value.name) {
    showCustomToast({
      message: 'Le nom du bailleur est requis',
      type: 'error',
    })
    return
  }
  ...
  await postData(API_ROUTES.CREATE_INFRA_BAILLEUR, payload)
    ...
    <div class="pb-6 mx-auto w-full max-w-7xl">
      ...
      <BoxPanelWrapper>
            ...
            <Dialog v-model:open="isModalOpen">
              <DialogTrigger as-child>
                <Button size="md" class="rounded-md max-sm:flex-1 sm:w-max">
                  ...
                  <span class="hidden sm:flex">Nouveau bailleur</span>
                </Button>
              </DialogTrigger>
              <DialogContent class="sm:max-w-[500px]">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Créer un nouveau bailleur d'infrastructure
                  </DialogDescription>
                </DialogHeader>
                <form @submit.prevent="handleSubmit" class="space-y-4">
                  <div class="space-y-2">
                    <Label for="name">Nom du bailleur<SpanRequired /></Label>
                    <Input
                      id="name"
                      v-model="formData.name"
                      type="text"
                      placeholder="Ex: ENABEL"
                      required
                  ...
                  <div class="space-y-2">
                    ...
                    <Textarea
                      ...
                      placeholder="Description du bailleur..."
                      class="border-gray-200"
                      rows="3"
                    />
                  </div>
        ...
        >
          ...
          <span>Chargement des bailleurs...</span>
        </div>
        <div
                ...
                <TableCell>
                  <div class="flex items-center gap-2">
                    ...
                    <AlertMessage
                      ...
                      message="Cette action est irréversible. Le bailleur sera définitivement supprimé."
                    >
                      <template #trigger>
                        <button
                          class="p-1.5 hover:bg-gray-100 rounded-md transition-colors opacity-0 group-hover:opacity-100"
