...
const breadcrumbItems = {
  items: [
    ...
    { label: 'Entreprises', isActive: true },
  ],
}

const activeTagName = 'entreprises'
const query = ref('')
...
// Local store (API n'a pas de GET list)
const entreprises = ref<any[]>([])

const { postData, loading, error, success } = usePostApi()

const filteredEntreprises = () => {
  if (!query.value) return entreprises.value
  const s = query.value.toLowerCase()
  return entreprises.value.filter((e: any) => e.name.toLowerCase().includes(s))
}

const resetForm = () => {
  form.value = { name: '', address: '', contact: '' }
...
const openEditDialog = (item: any) => {
  ...
  form.value = { name: item.name, address: item.address, contact: item.contact }
  dialogOpen.value = true
}
...
const handleSubmit = async () => {
  ...
  if (editingItem.value) {
    // Édition locale uniquement (pas de PUT API)
    const idx = entreprises.value.findIndex((e) => e.id === editingItem.value.id)
    if (idx !== -1) {
      entreprises.value[idx] = { ...entreprises.value[idx], ...form.value }
    }
    showCustomToast({ message: 'Entreprise modifiée localement.', type: 'success' })
    dialogOpen.value = false
    resetForm()
    return
  }

  await postData(API_ROUTES.CREATE_COMPANY, form.value)
  ...
  if (success.value) {
    ...
    entreprises.value.push(newItem)
    showCustomToast({ message: 'Entreprise créée avec succès.', type: 'success' })
    dialogOpen.value = false
    resetForm()
  } else if (error.value) {
...
const handleDelete = (id: number) => {
  ...
  showCustomToast({ message: 'Entreprise supprimée.', type: 'success' })
}
</script>
  ...
  <DashLayout :breadcrumb="breadcrumbItems" active-route="/insertion/prealables" module-name="insertion">
    <div class="pb-6 mx-auto w-full max-w-7xl h-full flex flex-col">
      ...
      <BoxPanelWrapper class="flex-1 flex flex-col min-h-0">
            ...
            <Dialog v-model:open="dialogOpen">
              <DialogTrigger as-child>
                <Button size="md" class="rounded-md max-sm:flex-1 sm:w-max" @click="openAddDialog">
                  ...
                  <span class="hidden sm:flex">Ajouter Entreprise</span>
                </Button>
              </DialogTrigger>
              <DialogContent class="sm:max-w-[460px]">
                <DialogHeader>
                  <DialogTitle>{{ editingItem ? 'Modifier l\'entreprise' : 'Nouvelle Entreprise' }}</DialogTitle>
                  <DialogDescription>
                    {{ editingItem ? 'Modifiez les informations.' : 'Renseignez les informations de l\'entreprise partenaire.' }}
                  </DialogDescription>
                </DialogHeader>
                <div class="grid gap-4 py-4">
                  <div class="grid gap-2">
                    <Label for="name">Nom <span class="text-red-500">*</span></Label>
                    <Input id="name" v-model="form.name" placeholder="Ex: RDC Telecom SA" />
                  </div>
                  <div class="grid gap-2">
                    <Label for="address">Adresse <span class="text-red-500">*</span></Label>
                    <Input id="address" v-model="form.address" placeholder="Ex: Av. de l'Industrie, Kinshasa" />
                  </div>
                  <div class="grid gap-2">
                    <Label for="contact">Contact <span class="text-red-500">*</span></Label>
                    <Input id="contact" v-model="form.contact" placeholder="Ex: +243 82 000 0000" />
                  </div>
                </div>
                <DialogFooter>
                  ...
                  <Button :disabled="loading" @click="handleSubmit">
                    ...
                    <span v-else>{{ editingItem ? 'Enregistrer' : 'Créer' }}</span>
                  </Button>
                </DialogFooter>
              </DialogContent>
            </Dialog>
        ...
        <!-- Table -->
        <div v-if="filteredEntreprises().length" class="mt-2 rounded-md overflow-hidden flex flex-1 bg-white">
          <Table class="rounded-md bg-white">
            ...
            <TableBody>
              <TableRow v-for="(item, index) in filteredEntreprises()" :key="item.id">
        ...
        <div v-else class="flex flex-col items-center justify-center h-full py-16 bg-white rounded-md text-gray-500">
          <span class="iconify hugeicons--building-04 text-5xl mb-3 text-gray-300"></span>
          <p class="font-medium">Aucune entreprise enregistrée</p>
          <p class="text-sm text-gray-400 mt-1">Commencez par ajouter une entreprise partenaire.</p>
        </div>
      </BoxPanelWrapper>
    </div>
  </DashLayout>
