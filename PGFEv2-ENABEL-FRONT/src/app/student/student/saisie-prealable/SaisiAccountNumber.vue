...
// import NewAccountNumber from '@/components/modals/NewAccountNumber.vue';
// import EditAccountNumber from '@/components/modals/EditAccountNumber.vue';
const { data, loading, error, fetchData } = useGetApi(API_ROUTES.GET_ACCOUNT_NUMBER_LIST)
...
eventBus.on('accountNumberUpdated', () => {
  fetchData({ page: page.value, limit: perPageCount.value })
})

//handleEdit
const handleEdit = (item: any) => {
  eventBus.emit('editAccountNumber', item)
}
const handleDelete = async (id: number) => {
  const url = API_ROUTES.DELETE_ACCOUNT_NUMBER.replace(':accountNumber', String(id))
  await deleteItem(url)
  eventBus.emit('accountNumberUpdated')
  if (errorDelete.value) {
    ...
    showCustomToast({
      message: 'Compte Numéro supprimé avec succès.',
      type: 'success',
    })
  }
}
...
<template>
  <LayoutSaisieOperation group="saisie" active-tag-name="account-number">
    <BoxPanelWrapper>
              ...
              <TableCell class="px-4">
                <div class="group flex items-center justify-between">
                  ...
                  <div class="hidden group-hover:flex items-center gap-2 ml-2">
                    ...
                    <AlertMessage
                      action="danger"
                      title="Supprimer un numéro de compte"
                      :message="`Vous êtes sur le point de supprimer le numéro de compte '${item.number}'. Êtes-vous sûr de continuer?`"
                    >
                      <template #trigger>
                        <Button
                          variant="ghost"
