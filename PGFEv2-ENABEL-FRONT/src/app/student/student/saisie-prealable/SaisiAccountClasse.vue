...
// import EditAccountClasse from '@/components/modals/EditAccountClasse.vue';
const { data, loading, error, fetchData } = useGetApi(API_ROUTES.GET_ACCOUNT_CLASS_LIST)
...
eventBus.on('accountClassUpdated', () => {
  fetchData({ page: page.value, limit: perPageCount.value })
})

//handleEdit
const handleEdit = (item: any) => {
  eventBus.emit('editAccountClass', item)
}

const handleDelete = async (id: number) => {
  const url = API_ROUTES.DELETE_ACCOUNT_CLASS.replace(':account', String(id))
  await deleteItem(url)
  eventBus.emit('accountClassUpdated')
  if (errorDelete.value) {
    ...
    showCustomToast({
      message: 'Compte de classe supprimé avec succès.',
      type: 'success',
    })
  }
}
...
<template>
  <LayoutSaisieOperation group="saisie" active-tag-name="account-class">
    <BoxPanelWrapper>
              ...
              <TableCell class="px-4">
                <div class="group flex items-center justify-between">
                  ...
                  <div class="hidden group-hover:flex items-center gap-2 ml-2">
                    ...
                    <AlertMessage
                      action="danger"
                      title="Supprimer un compte classe"
                      :message="`Vous êtes sur le point de supprimer le compte classe '${item.name}'. Êtes-vous sûr de continuer?`"
                    >
                      <template #trigger>
                        <Button
                          variant="ghost"
