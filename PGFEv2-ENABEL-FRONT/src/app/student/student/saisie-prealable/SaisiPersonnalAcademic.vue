...
// import NewAccountClasse from '@/components/modals/NewAccountClasse.vue';
const { data, loading, error, fetchData } = useGetApi(API_ROUTES.GET_PERSONNAL_ACADEMIC_LIST)
...
eventBus.on('personnalAcademicUpdated', () => {
  fetchData({ page: page.value, limit: perPageCount.value })
})

//handleEdit
const handleEdit = (item: any) => {
  eventBus.emit('editPersonnalAcademic', item)
}

const handleDelete = async (id: number) => {
  const url = API_ROUTES.DELETE_PERSONNAL_ACADEMIC.replace(':personnal', String(id))
  await deleteItem(url)
  eventBus.emit('personnalAcademicUpdated')
  if (errorDelete.value) {
    ...
    showCustomToast({
      message: 'Personnal scolaire supprimé avec succès.',
      type: 'success',
    })
  }
}
...
<template>
  <LayoutSaisieOperation group="saisie" active-tag-name="academic-personnal">
    <BoxPanelWrapper>
      <div class="flex items-center gap-3 justify-between">
        <div class="relative flex-1">
          <Input
            ...
            id="search"
            name="search"
            placeholder="Rechercher un personnal scolaire..."
        ...
        <div class="flex flex-wrap items-center gap-2.5">
          ...
          <Button size="md" class="rounded-md max-sm:flex-1 sm:w-max" as-child>
            <RouterLink to="/apprenants/operations/nouveau-personnel-academique">
              <span class="flex iconify hugeicons--plus-sign"></span>
              <span class="hidden sm:flex">Nouveau personnel scolaire</span>
            </RouterLink>
          </Button>
        </div>
      </div>
      ...
      >
        ...
        <span>Chargement des personnal scolaire...</span>
      </div>
              ...
              <TableCell class="px-4">
                <div class="group flex items-center justify-between">
                  ...
                  <div class="hidden group-hover:flex items-center gap-2 ml-2">
                    ...
                    <AlertMessage
                      action="danger"
                      title="Supprimer un personnel scolaire"
                      :message="`Vous êtes sur le point de supprimer le personnel scolaire '${item.name}'. Êtes-vous sûr de continuer?`"
                    >
                      <template #trigger>
                        <Button
                          variant="ghost"
