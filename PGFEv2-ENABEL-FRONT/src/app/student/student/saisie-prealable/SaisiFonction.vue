...
// import NewFonction from '@/components/modals/NewFonction.vue';
// import EditFonction from '@/components/modals/EditFonction.vue';
...
eventBus.on('fonctionUpdated', () => {
  fetchData({ page: page.value, limit: perPageCount.value })
})

//handleEdit
const handleEdit = (item: any) => {
  eventBus.emit('editFonction', item)
}
//handleDelete
const handleDelete = async (id: number) => {
  const url = API_ROUTES.DELETE_FONCTION.replace(':fonction', String(id))
  await deleteItem(url)
  eventBus.emit('fonctionUpdated')
...
<template>
  <LayoutSaisieOperation active-tag-name="fonction" group="saisie">
    <BoxPanelWrapper>
                ...
                <div class="group flex items-center justify-between">
                  ...
                  <div class="hidden group-hover:flex items-center gap-2 ml-2">
                    ...
                    <AlertMessage
                      ...
                      :message="`Vous êtes sur le point de supprimer la fonction '${item.title}'. Êtes-vous sûr de continuer?`"
                    >
                      <template #trigger>
                        <Button
                          variant="ghost"
