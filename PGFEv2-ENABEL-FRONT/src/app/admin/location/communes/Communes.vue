...
import NewCommunes from '@/components/modals/admin/location/NewCommunes.vue'
...
function openEditModal(c: Item) {
  ...
  editProvinceId.value = c.province_id

  // Find country of the province
  const p = (provinces.value || []).find((it: any) => String(it.id) === String(c.province_id))
...
async function onSubmitEdit() {
  ...
  await putData(`${API_ROUTES.GET_COMMUNES}/${editId.value}`, payload)
  if (updError.value) {
    showCustomToast({ message: updError.value, type: 'error' })
    return
  }
  if (updResponse.value) {
    showCustomToast({ message: 'Commune mise à jour avec succès', type: 'success' })
...
const activeTagName = computed(() => 'communes')
...
const { data, loading, error, fetchData } = useGetApi<Item[]>(API_ROUTES.GET_COMMUNES)
...
async function onDeleteCommune(id: number, name: string) {
  deletingId.value = id
  const res = await deleteItem(`${API_ROUTES.GET_COMMUNES}/${id}`)
  if (res) {
    showCustomToast({ message: `Commune "${name}" supprimée avec succès.`, type: 'success' })
    eventBus.emit('communeUpdated')
  } else {
...
onMounted(() => {
  ...
  fetchProvinces()
  reload()
  eventBus.on('communeUpdated', onRefresh)
})

onBeforeUnmount(() => {
  eventBus.off('communeUpdated', onRefresh)
})
          ...
          <template v-else>
            ...
            <div v-else class="w-full">
              ...
              >
                ...
                <div class="flex-1">Nom de la Commune</div>
              ...
              <ul class="divide-y">
                        ...
                        <AlertDialog>
                          ...
                          <AlertDialogContent>
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>
                                Action irréversible. Si des éléments dépendent de cette commune, la
                            ...
                            <AlertDialogFooter>
                              ...
                              <Button
                                ...
                                @click="onDeleteCommune(t.id, t.name)"
                              >
                                <span
                ...
                <Dialog v-model:open="editOpen">
                  <DialogContent class="sm:max-w-[480px]">
                    <DialogHeader>
                      <DialogTitle>Modifier une Commune</DialogTitle>
                      <DialogDescription
                        >Mettre à jour les informations de la commune</DialogDescription
                      >
                    </DialogHeader>
