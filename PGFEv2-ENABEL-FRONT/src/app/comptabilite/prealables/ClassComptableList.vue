...
interface Props {
  searchQuery: string
  filteredClasses: ClassComptable[]
...
<template>
  <BoxPanelWrapper>
    <div class="flex sm:items-center flex-col sm:flex-row sm:justify-between">
      <span class="text-gray-500 my-1.5 text-xl">Classes Comptables</span>
    </div>
    ...
    <div class="flex sm:items-center gap-3 flex-col sm:flex-row sm:justify-between">
      <div class="relative flex-1">
        <Input
          ...
          type="text"
          placeholder="Rechercher une classe comptable..."
      ...
      <div class="flex flex-wrap items-center sm:justify-end gap-2.5 flex-1">
        <Dialog :open="isCreateDialogOpen" @update:open="emit('update:isCreateDialogOpen', $event)">
          ...
          >
            <DialogHeader>
              <DialogTitle>Ajouter une classe comptable</DialogTitle>
              <DialogDescription>
                Créez une nouvelle classe comptable en remplissant les informations ci-dessous.
              </DialogDescription>
            </DialogHeader>
            <Form
              @submit="emit('submitCreate', $event)"
              :validation-schema="toTypedSchema(classComptableSchema)"
              class="space-y-4"
            >
              <div class="flex flex-col space-y-1.5">
                <Label for="create_code">Code <span class="text-red-500">*</span></Label>
                <Field name="code" v-slot="{ field, errorMessage }">
                  <Input v-bind="field" id="create_code" placeholder="Ex: CL001" maxlength="50" />
                  <span v-if="errorMessage" class="text-sm text-red-500">{{ errorMessage }}</span>
                </Field>
              </div>
              <div class="flex flex-col space-y-1.5">
                <Label for="create_name">Nom <span class="text-red-500">*</span></Label>
              ...
              <DialogFooter class="flex justify-end gap-2 items-center">
                ...
                <Button type="submit" size="sm" :disabled="creatingClass">
                  ...
                  {{ creatingClass ? 'Enregistrement...' : 'Enregistrer' }}
                </Button>
              </DialogFooter>
            </Form>
          </DialogContent>
    ...
    <div class="mt-4 rounded-md overflow-hidden flex flex-1 bg-white">
      <Table class="rounded-md bg-white">
        ...
        <TableBody>
          <TableRow v-if="classesLoading">
            <TableCell colspan="4" class="text-center py-8">
              <div class="flex items-center justify-center gap-2">
                <span class="iconify hugeicons--loading-03 animate-spin text-xl"></span>
                <span>Chargement des classes comptables...</span>
              </div>
            </TableCell>
          </TableRow>

          <TableRow v-else-if="classesError">
            <TableCell colspan="4" class="text-center py-8 text-red-500">
              <div class="flex items-center justify-center gap-2">
            ...
            <TableCell colspan="4" class="text-center py-8 text-gray-500">
              <div class="flex flex-col items-center gap-2">
                ...
                <span v-if="searchQuery"
                  >Aucune classe comptable trouvée pour "{{ searchQuery }}"</span
                >
                <span v-else>Aucune classe comptable enregistrée</span>
              </div>
            </TableCell>
          </TableRow>

          <TableRow v-else v-for="item in filteredClasses" :key="item.id">
    ...
    <TabPagination
      v-if="!classesLoading && !classesError"
      :model-value="page"
