...
// Validation schema based on backend contract (InputAccount)
...
const onSubmit = handleSubmit(async (values) => {
  try {
    await postData(API_ROUTES.CREATE_INPUTACCOUNT, values)
    ...
    if (response.value && (response.value as any).message) {
      showCustomToast({
        message: (response.value as any).message || 'Sortie créée avec succès',
        type: 'success',
      })
      resetForm()
    } else {
    ...
    router.push('/comptabilite/saisie-operations/recettes')
  } catch (e: any) {
...
<template>
  <ComptaLayout
    activeBread="Nouvelle Entree"
    active-tag-name="saisie-operations"
    group="operations"
    :show-header="false"
  >
    <form @submit.prevent="onSubmit" class="mt-10 space-y-8">
      <FormSection
        ...
        title="Informations sur la sortie"
      >
        <InputWrapper>
          <Label class="text-sm">
      ...
      <div class="flex items-center justify-end gap-2">
        <Button
          ...
          type="button"
          :disabled="loading"
          @click="router.push('/comptabilite/saisie-operations/recettes')"
        >
          <span class="flex iconify hugeicons--cancel-01 mr-1.5"></span>
          Annuler
        </Button>
        <Button type="submit" :disabled="loading">
          <span class="flex iconify hugeicons--add-01 mr-2"></span>
          <span v-if="!loading">Enregistrer une entree</span>
