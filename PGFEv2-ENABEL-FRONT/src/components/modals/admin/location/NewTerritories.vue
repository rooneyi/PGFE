...
const schemaForm = z.object({
  name: z
    .string({ required_error: 'Veuillez saisir le nom du territoire' })
...
const onSubmit = handleSubmit(async (values) => {
  ...
  await postData(API_ROUTES.CREATE_TERRITORY, payload)
  ...
  if (success.value) {
    ...
    showCustomToast({
      message: response.value?.message || 'Territoire ajouté avec succès',
      type: 'success',
    })
    resetForm()
    open.value = false
  ...
  <Dialog v-model:open="open">
    <DialogTrigger as-child>
      <Button size="md" class="rounded-md">
        ...
        <span class="hidden sm:flex"> Nouveau territoire </span>
      </Button>
    </DialogTrigger>
    <DialogContent class="sm:max-w-[540px]">
      <DialogHeader>
        <DialogTitle>Nouveau territoire</DialogTitle>
        <DialogDescription> Enregistrer un territoire </DialogDescription>
      </DialogHeader>
        ...
        <div class="grid gap-4 py-4">
          <div class="flex flex-col space-y-1.5">
            ...
            <Input
              ...
              name="name"
              v-model="name"
              placeholder="Entrez le nom du territoire"
